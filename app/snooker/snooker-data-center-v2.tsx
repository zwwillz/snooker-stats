...
function EventTableHeader() {
  ...
  useEffect(() => {
    ...
    const syncObserver = () => {
      ...
      observer = new IntersectionObserver(([entry]) => setPinned(entry.boundingClientRect.top <= 64), { root: null, rootMargin: "-64px 0px 0px 0px", threshold: 0 });
...
export default function SnookerDataCenterV2({
  ...
  if (detail?.type === "player") {
    ...
    return <main className={styles.appRoot} data-theme={theme}>{detailSiteHeader("players")}<div className={`${styles.detailShell} ${priority.playerDetailShell}`}>
  ...
  if (detail?.type === "ranking") {
    return <main className={styles.appRoot} data-theme={theme}>{detailSiteHeader("data")}<div className={`${styles.detailShell} ${priority.dataDetailShell}`}>
      <header className={`${styles.detailHeader} ${priority.detailLocalHeader}`}> <button onClick={closeRankings}>‹</button><strong>排名中心</strong><span>STATS</span></header>
