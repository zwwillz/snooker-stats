...
test("technical leaderboard is a desktop table pilot while mobile keeps the compact list", async () => {
  ...
  assert.match(css, /grid-template-areas:"gutter gutter" "sidebar table"/);
  ...
  assert.match(css, /\.technicalDesktopIntro\{display:grid;grid-template-columns:minmax\(0,1fr\) auto;gap:16px;align-items:center;padding:24px 2px 0\}/);
  assert.doesNotMatch(css, /\.technicalPage::before/);
  assert.match(css, /\.technicalStickyGutter\{grid-area:gutter;position:sticky;top:var\(--snooker-header-height,68px\);z-index:4;height:var\(--technical-sticky-gap\)/);
  assert.match(css, /\.technicalSidebar\{position:sticky;top:calc\(var\(--snooker-header-height,68px\) \+ var\(--technical-sticky-gap\)\)/);
  assert.match(css, /\.technicalTableStickyHead\{position:sticky;top:calc\(var\(--snooker-header-height,68px\) \+ var\(--technical-sticky-gap\)\);z-index:2;margin:0 -18px;background:transparent\}/);
