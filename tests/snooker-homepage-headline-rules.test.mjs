...
function select(matches, now = NOW, limit = 4) {
  const event = { id: "event-1", rounds: [{ key: "round-1", matches }] };
...
test("the forward migration stamps future terminal transitions without historical backfill", async () => {
  const migration = await readFile(new URL("../supabase/migrations/20260827184950_stamp_future_terminal_match_detection.sql", import.meta.url), "utf8");
  ...
  assert.doesNotMatch(migration, /update public\.snooker_matches/);
});
