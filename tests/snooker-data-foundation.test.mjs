...
test("phase 4 event and match details keep desktop hierarchy without widening the data load", async () => {
  ...
  assert.doesNotMatch(uiSource, /eventHeaderCompact/);
  assert.doesNotMatch(uiSource, /data-event-header-state/);
  assert.doesNotMatch(uiSource, /eventStickyNav/);
  assert.doesNotMatch(uiSource, /eventStickyIdentity/);
  assert.doesNotMatch(uiSource, /matchBackButton/);
  assert.match(uiSource, /aria-label="返回上一页"/);
  ...
  assert.match(priorityCss, /TOURNAMENTS_PHASE4_FOLLOWUP_V1/);
  assert.match(priorityCss, /grid-template-columns:1fr!important/);
  assert.match(priorityCss, /\.eventDetailTabs\{position:sticky!important;top:var\(--snooker-header-height\)!important/);
