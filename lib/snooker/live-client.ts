...
function statePriority(match: SnookerMatch, now: number) {
  ...
  if (FINAL_STATUSES.has(match.status)) {
    ...
    return completedAt && now - completedAt <= COMPLETED_PROTECTION_MS ? 300 : 0;
  }
...
function compareHeadlineCandidates(a: HeadlineSelection, b: HeadlineSelection, players: Map<string, SnookerPlayer>, now: number) {
...
function sortHeadlineCandidates(candidates: HeadlineSelection[], players: Map<string, SnookerPlayer>, now: number) {
  return candidates.sort((a, b) => compareHeadlineCandidates(a, b, players, now));
}
...
export function selectHomepageHeadlineMatches(
  ...
  if (live.length) {
    sortHeadlineCandidates(live, players, now);
  ...
  if (recentlyCompleted.length) {
    recentlyCompleted.sort((a, b) => resolveCompletedAt(b.match, now) - resolveCompletedAt(a.match, now)
      || compareHeadlineCandidates(a, b, players, now));
  ...
  upcoming.sort((a, b) => time(a.match.scheduledAt) - time(b.match.scheduledAt)
    || compareHeadlineCandidates(a, b, players, now));
