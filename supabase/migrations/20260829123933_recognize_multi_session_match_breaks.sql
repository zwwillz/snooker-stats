...
declare
  ...
  v_future_session_resume boolean := false;
...
begin
  ...
  v_future_session_resume :=
    ...
    and coalesce(new.score1, 0) + coalesce(new.score2, 0) > 0
    and greatest(coalesce(new.score1, 0), coalesce(new.score2, 0)) < v_win_target;

  if new.status not in ('completed', 'walkover') then
    if v_future_session_resume then
      new.status := 'session-break';
      new.current_player_side := null;
      new.current_break := null;
    elsif v_source in ('suspended', 'paused', 'interrupted') then
      new.status := 'session-break';
      new.current_player_side := null;
      new.current_break := null;
  ...
  if tg_op = 'UPDATE' then
    if old.status = 'walkover' and new.status = 'completed' then
      new.status := 'walkover';
      new.winner_id := coalesce(old.winner_id, new.winner_id);
      new.completed_detected_at := coalesce(old.completed_detected_at, new.completed_detected_at);
      new.realtime_finalized_at := coalesce(old.realtime_finalized_at, new.realtime_finalized_at);
      new.frames_complete := true;
      new.note := coalesce(old.note, new.note);
    end if;
